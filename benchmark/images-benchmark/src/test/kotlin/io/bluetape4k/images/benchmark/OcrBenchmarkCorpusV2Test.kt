...
class OcrBenchmarkCorpusV2Test {
    ...
    fun `v2 manifest verifies image ground truth geometry and receipts`() {
        ...
        fixture.boxes.map(OcrBenchmarkCorpusBox::order).distinct().size.shouldBeEqualTo(6)
    }
    ...
    fun `v2 loader rejects duplicate geometry order`() {
        ...
        val invalidBoxes = resources.getValue(boxesPath).decodeToString()
    ...
    private fun canonicalManifest(): String =
        resource("bench/ocr-v2/manifest.json").decodeToString()
