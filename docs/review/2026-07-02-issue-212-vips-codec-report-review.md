# 검토 - Issue #212 VIPS Codec Capability Report

날짜: 2026-07-02
범위: `images-vips-api`, `images-vips-java21`, and `images-vips-java25` AVIF/HEIC capability reporting and smoke helpers.

## 검토 관점

| 관점 | 판정 | 근거 |
|---|---|---|
| Tier 4 - 정확성 | PASS | `VipsCodecSupport` distinguishes `AVAILABLE`, `UNAVAILABLE`, and `UNKNOWN`; Java 21 reports JVips introspection gaps as `UNKNOWN`; Java 25 probes `heifload_buffer` and `heifsave_buffer`. |
| Tier 5 - 테스트 범위 | PASS | API, Java 21, and Java 25 tests cover stable formats, backend-specific HEIF-family capability states, sanitized smoke failures, and smoke-result invariants. |
| Tier 7 - 문서 | PASS | Root, API, Java 21, and Java 25 READMEs are updated in English and Korean with report/smoke guidance. |
| 공개 API/KDoc | PASS | New public enums/data classes and `VipsRuntime` functions have English KDoc and use serializable model types. |
| bluetape4k 패턴 | PASS | Tests use bluetape4k assertions; Java 25 probe tests use a field seam reset in lifecycle methods instead of repeated MockK operation mocking. |
