...
`@MustBeDocumented`, binary-retained annotation in the Vips API package. Match
the existing marker's supported targets exactly: `CLASS`, `ANNOTATION_CLASS`,
`FUNCTION`, `PROPERTY`, `PROPERTY_GETTER`, `PROPERTY_SETTER`, `CONSTRUCTOR`,
and `TYPEALIAS`. Its public KDoc and compiler message are English and state
that Vips codec capability APIs may change without binary-compatibility
guarantees, with the exact `@OptIn(VipsIncubatingApi::class)` migration path.

Replace `IncubatingImageApi` usage throughout the Vips API and its Java 21/25
backend consumers, including main and test source sets:

- Apply `@VipsIncubatingApi` to `VipsImageFormat.AVIF` and
  `VipsImageFormat.HEIC`. These are the public declarations that require
  caller opt-in.
- Replace the existing implementation-only `@OptIn(IncubatingImageApi::class)`
  on `VipsCodecCapability` and `VipsCodecCapabilityReport` with
  `@OptIn(VipsIncubatingApi::class)`. Do not apply `@VipsIncubatingApi` to
  those report container types: callers must still be able to inspect stable
  codec report data without accepting an unnecessary new opt-in contract.
- All `images-vips-api` and Java 21/25 backend main-source and test `@OptIn`
  declarations that use those Vips capability types. The backend modules depend
  on `images-vips-api`, not directly on `bluetape4k-images`, so retaining the
  old import would break their compilation after the main boundary is removed.
...
No Vips API or backend main/test source may import
`io.bluetape4k.images.IncubatingImageApi` after the migration. Existing
`bluetape4k-core`, IO, Okio, and coroutine dependencies remain intentional
Vips API dependencies.
...
- update every AVIF/HEIC example, including capability and smoke snippets, with
  the new `VipsIncubatingApi` import and a scoped
...
Replace the Vips-specific snippets in the `images` module's `AvifWriter` and
`HeicReader` KDoc with contract-only examples or prose. Those interfaces remain
guarded by `IncubatingImageApi`, but the `images` module must not import the
Vips-owned marker or acquire a reverse dependency merely to document a Vips
example.

## Verification Design

1. Compile and run `:bluetape4k-images-vips-api:test`.
2. Compile both backend consumers' main and test source sets
   ...
   `:bluetape4k-images-vips-java25:compileTestKotlin`) to prove they use the
   revised public contract without requiring native-runtime test execution.
3. Generate the Vips API publication POM and Gradle Module Metadata with
   `:bluetape4k-images-vips-api:generatePomFileForBluetapeImagePublication`
   and `:bluetape4k-images-vips-api:generateMetadataFileForBluetapeImagePublication`.
4. Assert that the generated main POM and Gradle Module Metadata's normal
   `apiElements`/`runtimeElements` variants have no dependency on
   `bluetape4k-images`, Scrimage, or TwelveMonkeys; retain an allow-list of
   intentional API dependencies. The metadata assertion must distinguish the
   intentionally dependent test-fixtures variant from normal consumer variants.
5. Add dedicated Kotlin compiler fixtures without a new dependency: compile an
   unopted AVIF/HEIC use with warnings promoted to errors and assert its
   expected failure mentions `VipsIncubatingApi`; compile the opted counterpart
   with the same strict setting and assert success. Retain capability model
   behavior tests without imposing the marker on stable report containers.
6. Verify English/Korean README and affected `images` KDoc claims against the
   generated publication metadata and current source names, then run
   `git diff --check`.
...
| A fixture dependency leaks into a normal consumer variant | Inspect the generated main POM and Gradle Module Metadata, distinguishing fixture variants from `apiElements`/`runtimeElements`. |
| Public annotation migration is accidentally broadened to Scrimage APIs | Restrict marker changes to Vips capability opt-ins in `images-vips-api` and Java 21/25 backends; keep `IncubatingImageApi` definitions and image API contracts in `images`. |
| Runtime codec behavior changes during the refactor | Limit backend changes to opt-in annotation imports; retain existing codec tests and run targeted Vips API tests. |
| Publication metadata uses a different configuration than local resolution | Use the actual `BluetapeImage` POM and Gradle Module Metadata generation tasks as the acceptance evidence. |
...
- Vips AVIF/HEIC capability APIs remain explicitly opt-in with a public,
  Vips-scoped marker.
- Stable codec report containers remain usable without the Vips opt-in; only
  AVIF/HEIC enum entries propagate the marker to callers.
...
Before publication, reverting the feature branch restores the existing POM,
Gradle Module Metadata, and annotation contract. If generated-POM validation or
any normal Gradle metadata variant reveals an unaccounted public-type or
forbidden dependency, stop before PR creation, retain the evidence, and revise
the spec/plan instead of reintroducing the broad `images` API dependency by
default.
