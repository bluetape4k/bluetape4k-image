...
def diagrams() -> tuple[FlowDiagram | SequenceDiagram, ...]:
    return (
        FlowDiagram(
            ...
            height=1100,
            nodes=(
                Node("app", "Spring Boot app", ("imports starter",), 360, 205, 300, 78, 0),
                Node("props", "Images properties", ("bluetape4k.images.*",), 795, 205, 330, 78, 1),
                Node("autoProcessing", "Processing Config", ("AutoConfiguration", "format and quality"), 240, 405, 280, 86, 0),
                Node("autoStorage", "Storage Config", ("AutoConfiguration", "local or S3 backend"), 590, 382, 300, 86, 2),
                Node("autoCdn", "CDN Config", ("AutoConfiguration", "S3 or CloudFront"), 970, 420, 285, 86, 3),
                Node("autoHealth", "Health Config", ("AutoConfiguration", "reactive probe"), 1285, 370, 285, 86, 4),
                Node("autoMetrics", "Metrics Config", ("AutoConfiguration", "Micrometer wrapper"), 1420, 486, 285, 86, 5),
                Node("local", "Local storage", ("filesystem root", "path traversal guard"), 360, 625, 300, 86, 4),
                Node("s3", "S3 storage", ("S3Operations", "bucket + key prefix"), 760, 605, 300, 86, 2),
                Node("signers", "Read URL signers", ("pre-signed S3", "CloudFront URLs"), 1125, 640, 300, 86, 3),
                Node("storageSpi", "ImageStorage SPI", ("upload, download, exists",), 1415, 635, 300, 86, 1),
                Node("health", "Storage health", ("exists(healthProbeKey)",), 340, 842, 340, 84, 4),
                Node("metrics", "Metrics wrapper", ("timers and counters",), 790, 850, 340, 84, 5),
                Node("actuator", "Actuator sanitizer", ("privateKeyPem redaction",), 1245, 838, 340, 84, 6),
            ),
            edges=(
                Edge("app", "autoProcessing", "", ((510, 283), (510, 340), (380, 340), (380, 405)), "#56708C"),
                Edge("props", "autoStorage", "", ((960, 283), (960, 340), (740, 340), (740, 382)), "#8A72D6", True),
                Edge("autoProcessing", "autoStorage", "", ((520, 448), (590, 425)), "#56708C"),
                Edge("autoStorage", "autoCdn", "", ((890, 425), (970, 463)), "#DB7890"),
                Edge("autoStorage", "local", "", ((740, 468), (740, 555), (510, 555), (510, 625)), "#45A7A1"),
                Edge("autoStorage", "s3", "", ((740, 468), (740, 560), (910, 560), (910, 605)), "#D9AA4D"),
                Edge("autoCdn", "signers", "", ((1112, 506), (1112, 580), (1275, 580), (1275, 640)), "#DB7890"),
                Edge("autoStorage", "storageSpi", "", ((890, 425), (1360, 425), (1360, 678), (1415, 678)), "#58A978"),
                Edge("autoHealth", "health", "", ((1428, 456), (1428, 760), (510, 760), (510, 842)), "#45A7A1"),
                Edge("autoMetrics", "metrics", "", ((1562, 572), (1562, 790), (960, 790), (960, 850)), "#8A72D6"),
                Edge("autoCdn", "actuator", "", ((1112, 506), (1112, 790), (1415, 790), (1415, 838)), "#B88A44"),
                Edge("local", "metrics", "", ((510, 711), (510, 800), (960, 800), (960, 850)), "#45A7A1"),
                Edge("s3", "metrics", "", ((910, 691), (910, 800), (960, 800), (960, 850)), "#D9AA4D"),
            ),
            ...
            panels=(
                Panel("Application", 72, 176, 1656, 132, 0),
                Panel("Auto-Configuration", 72, 356, 1656, 196, 1),
                Panel("Runtime Services", 72, 588, 1656, 160, 2),
                Panel("Operations", 72, 806, 1656, 148, 4),
            ),
            javers_style=True,
        ),
        FlowDiagram(
            ...
            height=1100,
            nodes=(
                Node("app", "Ktor application", ("install core plugin",), 370, 205, 300, 78, 0),
                Node("core", "Ktor core baseline", ("JSON and API errors",), 805, 205, 330, 78, 1),
                Node("routing", "Routing DSL", ("routing { ... }",), 300, 382, 300, 86, 2),
                Node("thumb", "Thumbnail routes", ("multipart upload", "PNG thumbnail"), 720, 405, 320, 86, 3),
                Node("captchaRoutes", "CAPTCHA routes", ("issue and verify",), 1085, 374, 320, 86, 4),
                Node("errors", "Error payloads", ("ApiErrorResponse",), 1390, 440, 300, 86, 5),
                Node("images", "bluetape4k-images", ("immutableImageOf", "max + writer"), 400, 630, 340, 86, 5),
                Node("captcha", "images-captcha", ("CaptchaGenerator", "VerificationService"), 845, 606, 360, 86, 6),
                Node("challengeStore", "Challenge store", ("in-memory default", "replace for clusters"), 1260, 642, 340, 86, 2),
                Node("auth", "Authorization policy", ("host application",), 360, 850, 320, 78, 0),
                Node("cdn", "S3/CDN URLs", ("compose outside routes",), 790, 838, 320, 78, 2),
                Node("native", "Native acceleration", ("libvips remains optional",), 1210, 852, 340, 78, 4),
            ),
            edges=(
                Edge("app", "routing", "", ((520, 283), (520, 340), (450, 340), (450, 382)), "#56708C"),
                Edge("core", "errors", "", ((970, 283), (970, 335), (1540, 335), (1540, 440)), "#8A72D6", True),
                Edge("routing", "thumb", "", ((600, 425), (720, 448)), "#56708C"),
                Edge("routing", "captchaRoutes", "", ((600, 425), (675, 425), (675, 548), (1245, 548), (1245, 460)), "#58A978"),
                Edge("thumb", "images", "", ((880, 491), (880, 565), (570, 565), (570, 630)), "#DB7890"),
                Edge("captchaRoutes", "captcha", "", ((1245, 460), (1245, 565), (1025, 565), (1025, 606)), "#45A7A1"),
                Edge("captchaRoutes", "challengeStore", "", ((1245, 460), (1245, 585), (1430, 585), (1430, 642)), "#D9AA4D"),
                Edge("routing", "auth", "", ((450, 468), (450, 810), (520, 810), (520, 850)), "#56708C", True),
                Edge("images", "cdn", "", ((570, 716), (570, 805), (950, 805), (950, 838)), "#D9AA4D", True),
                Edge("images", "native", "", ((570, 716), (570, 805), (1380, 805), (1380, 852)), "#45A7A1", True),
            ),
            ...
            panels=(
                Panel("Application", 72, 176, 1656, 132, 0),
                Panel("Route Helpers", 72, 356, 1656, 176, 1),
                Panel("Processing Libraries", 72, 588, 1656, 160, 2),
                Panel("Host Responsibilities", 72, 806, 1656, 148, 4),
            ),
            javers_style=True,
        ),
        FlowDiagram(
