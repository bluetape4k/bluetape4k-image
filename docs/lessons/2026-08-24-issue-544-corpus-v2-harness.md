...
byte sequence를 decoder에 전달하고, `ocr-boxes-v1`의 schema·uniqueness·pixel
bounds를 fail-closed로 검사한다. malformed bytes는 정상 `TEXT` fixture가 아닌
`DECODE_FAILED` negative receipt로 분리했다.

## 결과

기존 `clean-text.png`를 재사용한 대표 v2 fixture와 별도 text·boxes·schema·generator
receipt를 추가했다. 현재 구현은 계약 harness의 최소 slice이며, 9개 시나리오·최소
27개 fixture와 실제 Tesseract/Paddle 비교 수치는 아직 후속 benchmark train의
PENDING gate다.

## 검증

- `./gradlew :bluetape4k-images-benchmark:test --tests 'io.bluetape4k.images.benchmark.OcrBenchmark*' --console=plain`: 10/10 PASS
